...
public class Calculator {
    public static double calculate(Queue<Pair<String, Double>> polish_notation_) {
        Stack<Double> intermediate_result = new Stack<>();
        while (!polish_notation_.isEmpty()) {
            String operation = polish_notation_.peek().getKey();
            double value = Objects.requireNonNull(polish_notation_.poll()).getValue();
            if (Objects.equals(operation, "num")) {
                intermediate_result.push(value);
            } else if (isBinaryOperation(operation)) {
                executionBinaryOperation(intermediate_result, operation);
            } else {
                executionUnaryOperation(intermediate_result, operation);
            }
